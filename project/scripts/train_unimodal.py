...
def parse_args():
    ...
    parser.add_argument("--modality", type=str, required=True, choices=["text", "audio", "vision"])
...
def main():
    ...
    input_dim = get_input_dim(args.modality)
    model = UnimodalLSTM(
        input_dim=input_dim,
    ...
    best_epoch = -1
    best_ckpt_path = os.path.join(args.save_dir, f"best_{args.modality}.pt")
    patience_counter = 0

    for epoch in range(1, args.epochs + 1):
        train_loss = train_one_epoch(
            model, train_loader, optimizer, criterion, device, args.modality
        )
        val_metrics = evaluate(
            model, valid_loader, criterion, device, args.modality
        )
